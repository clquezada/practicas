classfunction<-function(datos=x)
{
  ...
  for (i in 1:datos$n_sources)
  {
    ...
    superdata[var_names3[i]]<-list(meanSD(25,datos$correction_means[i,2],datos$source_sds[i,2]))
    
    superdata[var_names4[i]]<-list(meanSD(25,datos$correction_means[i,1],datos$source_sds[i,1]))
